...
struct MainWindow : Window 
{
                   ...
                   circuit(nullptr), prev_ui_state{false, false, false, false}
    {
        ...
        status_visible_item.setChecked(true);
            ...
            while(SDL_PollEvent(&event))
            {
                ...
                {   
                    case SDL_JOYBUTTONDOWN:
                        input_window.active_selector->assign({KeyAssignment::JOYSTICK_BUTTON, event.jbutton.button, static_cast<unsigned int>(event.jbutton.which)});
                        break;
                    case SDL_JOYAXISMOTION:
                        ...
                        if(abs(event.jaxis.value) > 8192)
                        {
                            ...
                            input_window.active_selector->assign({KeyAssignment::JOYSTICK_AXIS, axis, static_cast<unsigned int>(event.jaxis.which)});
                        }
                    default:
                        break;
                }
...
int main(int argc, char** argv)
{
    ...
    if(argc > 1)
    {
        ...
        for(const GameDesc& g : game_list)
        {
            ...
            {
                if (start_fullscreen) main_window.toggleFullscreen(true);
